...
日期时间字符串、日期时间对象和Unix时间戳的相互转换方式和对应转换函数如下图所示。![转化方式](../images/p58764.png) 上图所示的转换场景和对应的转换函数具体描述如下表所示。
...
**说明** 了解更多日期时间处理和转换函数，请参见[日期时间函数](t908955.html#concept-1130519)和[转换函数](t908949.html#concept-1130510)。
...
* 场景1：不带时区信息的日期时间字符串类型转换为Unix时间戳。对于不带时区信息的日期时间字符串如`2019-06-02 18:41:26`，将日期时间转化为Unix时间戳，需要指定该日期时间的时区，不同的时区转化得到的Unix时间戳的值不一样。
  ...
  * 时区参数`tz=时区字符串`中所有可选时区字符串请参见[时区列表](t1267144.html#concept-1597620)
...
* 处理函数
  ...
  * `dt_add`中`weekday`参数通常和`dt_MO`，`dt_TU`等参数一起使用，表示特定星期几的偏移，如下例所示。具体请参见[dt_MO](t908955.html#concept-1130519/section-bm9-1dd-jnv)。 
